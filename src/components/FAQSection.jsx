    ...
    <section className="bg-white py-12 justify-items-start sm:py-16 lg:py-20">
      <div className="mx-auto max-w-screen-2xl px-4 md:px-8">
        ...
        <div className="mb-12 md:mb-16 text-center">
          <h2 className="mb-4 text-3xl font-bold text-[#243137] text-[55px]  inline-block rounded-2xl">
        ...
        {/* FAQ Items */}
        <div className="mx-auto max-w-screen-sm divide-y divide-gray-800 border-t text-black border-gray-800">
          {faqs.map((faq, index) => (
            <div key={index} className="border-b border-gray-800">
              <button
                onClick={() => toggleFAQ(index)}
                className="flex w-full items-center justify-between gap-2 py-4 text-left font-semibold text-balck transition duration-200 hover:text-indigo-400"
              >
              ...
              {openIndex === index && (
                <p className="mb-4 text-gray-800">{faq.answer}</p>
              )}
            </div>
          ))}
        </div>
